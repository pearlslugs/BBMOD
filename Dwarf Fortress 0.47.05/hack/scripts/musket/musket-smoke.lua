...
local weaponProperties = { --currently just for muzzle smoke amounts. will turn into 2d array if needed for more stuff in future but trying to avoid too many table lookups
	--normal weapons
	["ITEM_WEAPON_GUN_MUSKET"] = defaultSmoke,
	["ITEM_WEAPON_GUN_MUSKET_BAYONET"] = defaultSmoke,
	["ITEM_WEAPON_GUN_PISTOL"] = defaultSmoke,
	["ITEM_WEAPON_GUN_PISTOL_BAYONET"] = defaultSmoke,
	
	--shotgun weapons
	["ITEM_WEAPON_GUN_BLUNDERBUSS"] = defaultSmoke,
	["ITEM_WEAPON_GUN_DRAGON_PISTOL"] = defaultSmoke,
	
	--explosive weapons
	["ITEM_WEAPON_RIFLE_AUTO"] = defaultSmoke,
	["ITEM_WEAPON_GUN_ROCKETLAUNCHER"] = {5, 20, 20},
...
eventful.onProjItemCheckMovement.musket = function(projectile)
	if df.item.find(projectile.bow_id) ~= nil then --fucking explosions destroying weapons
		...
		if string.find(weaponType, "AUTO", 11) then
			...
			elseif weaponType == "ITEM_WEAPON_RIFLE_AUTO" then
...
function weaponMod(projectile, weaponType)
	if weaponType == "ITEM_WEAPON_GUN_AUTOGUN" then 
		setFireRate(projectile, 15)
		setInaccuracy(projectile, 10)
	elseif weaponType == "ITEM_WEAPON_GUN_BLUNDERBUSS" or weaponType == "ITEM_WEAPON_GUN_DRAGON_PISTOL" then
